...
int8_t sr_nvm_read(radio_t *radio, uint8_t *buf, uint8_t addr_start, uint8_t addr_end)
{
    ...
    while (addr_current <= addr_end) {
        sr_access_write_reg(radio->radio_id, REG8_ROM, CONNECT_ROM_PWR | addr_current++);
        read_reg = sr_access_read_reg(radio->radio_id, REG8_ROM);
        buf[idx++] = read_reg;
    }
...
{
    uint8_t *ptr = NULL;
    int8_t pulse_width_offset = 0, pwo_temp = 0;
...
{
    uint8_t *ptr = NULL;
    int8_t vref_tune_offset = 0, vto_temp = 0;
...
{
    uint8_t *ptr = NULL;
    uint8_t width_freq = 0;
    uint8_t width_code = 0;
...
uint8_t sr_nvm_get_width_base_freq(nvm_t *nvm)
{
    uint8_t *ptr = NULL;
    uint8_t width_freq = 0;
    uint8_t base_freq_at_room_temp = 0;
...
{
    uint8_t *ptr = NULL;
    uint32_t wafer_value = 0;
    uint16_t wafer_lot = 0;
...
void sr_nvm_power_up(radio_t *radio)
{
    ...
    do {
        /* Wake up the ROM */
        sr_access_write_reg(radio->radio_id, REG8_ROM, CONNECT_ROM_PWR);
        read_reg = sr_access_read_reg(radio->radio_id, REG8_PWR_STATUS_COMMANDS);
    } while (!GET_ROM_EN(read_reg));
    sr_utils_wait_delay(NVM_DELAY_AFTER_ROMEN_SET_MS);
}

void sr_nvm_power_down(radio_t *radio)
{
    sr_access_write_reg(radio->radio_id, REG8_ROM, 0x00);
}
