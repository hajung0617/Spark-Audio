...
typedef enum nvm_entry_key {
    ...
    /*! Calibration NVM entry key, 8-bit value with field : PLL_RES[6..4] and VREF_TUNE[3..0]. */
    NVM_KEY_CALIBRATION = 0x03,
    /*! Pulse width and base frequency NVM entry key, 8-bit value with field : WIDTH_CODE[7..5] and FREQ_CODE[4..0]. */
    NVM_KEY_WIDTH_FREQ = 0x04,
    /*! Frequency ticks NVM entry key, 8-bit value */
    NVM_KEY_FREQ_TICKS = 0x05,
    /*! Wafer lot and ID NVM entry key, 24-bit value with field : WAFER_LOT_0[7..0], WAFER_LOT_1[15..8] and
     * WAFER_ID[20..16].
     */
    NVM_KEY_WAFER_LOT = 0x06,
    /*! Test date NVM entry key, 24-bit value with field : Date[23..19], month[15..12], year[7..1] and century[0]. */
    NVM_KEY_TEST_DATE = 0x07,
    /*! Test program version NVM entry key, 16-bit value */
    NVM_KEY_TP_VERSION = 0x08,
    /*! Region corner NVM entry key, 8-bit value with field : REGION[7..4] and CORNER[3..0]. */
    NVM_KEY_REG_CORNER = 0x09,
    /*! Product ID NVM entry key, 16-bit value */
    NVM_KEY_PRODUCT_ID = 0x0A,
    /*! VCRO Shift NVM entry key, 16-bit value with field : FREQ_SHIFT_RX_1[15..12], FREQ_SHIFT_RX_0[11..8],
     *   FREQ_SHIFT_TX_1[7..4] and FREQ_SHIFT_TX_0[3..0].
     */
    NVM_KEY_VCRO_SHIFT = 0x0B,
    /*! Offset values for VREF_TUNE and PULSE_WIDTH : PULSE_WIDTH_OFFSET[7..4], VREF_TUNE_OFFSET[3..0].
     *   Each 4-bit value is signed(two's complement)
     */
    NVM_KEY_VREF_ADJUST = 0x0C,
    /*! NVM delimiter */
    NVM_KEY_LAST,
