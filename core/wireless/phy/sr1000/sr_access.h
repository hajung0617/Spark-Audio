 ...
 */
static inline void sr_access_write_reg(uint8_t radio_id, uint8_t reg, uint8_t value)
{
    uint8_t tx_buffer[2];
    uint8_t rx_buffer[2];
...
static inline void sr_access_write_reg16(uint8_t radio_id, uint8_t reg, uint16_t value)
{
    ...
    for (uint8_t i = 0; i < byte_count; i++) {
        sr_access_write_reg(radio_id, reg + i, EXTRACT_BYTE(value, byte_count - (i + 1)));
    }
}

/** @brief Write 32 bits into 4 concecutive registers.
 *
 *  @param[in] radio_id  Radio HAL structure array index
 *  @param[in] reg       Register to write.
 *  @param[in] value     Value to write.
 */
static inline void sr_access_write_reg32(uint8_t radio_id, uint8_t reg, uint32_t value)
{
 ...
 */
static inline uint8_t sr_access_read_reg(uint8_t radio_id, uint8_t reg)
{
    uint8_t tx_buffer[2];
    uint8_t rx_buffer[2];
