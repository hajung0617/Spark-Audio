 ...
 */
typedef struct {
    /*! Flag use to know if calibration value is better from a list */
    bool lower_than_target;
    /*! DCRO calibration code */
    uint8_t code;
} dcro_calib_t;
...
typedef struct {
    ...
    uint8_t phy_model;
    /*! Package of the transceiver */
    uint8_t phy_package;
    /*! Product id version */
    uint8_t phy_version;
    /*! Code identifying the binning setup used for this chip */
    uint16_t binning_setup_code;
    /*! Resistance tuning value to calibrate the PLL and band gap*/
    uint8_t resistune;
    /*! Low noise amplifier peak frequency table */
    const uint16_t *lna_peak_table;
    /*! Corrected DCRO codes to use for TX */
    uint8_t new_dcro_codes_tx[DCRO_CODE_COUNT_MAX];
    /*! Corrected DCRO codes to use for RX */
    uint8_t new_dcro_codes_rx[DCRO_CODE_COUNT_MAX];
    /*! Number of corrected DCRO codes to use for RX */
    uint8_t new_dcro_codes_rx_size;
    /*! Number of Corrected DCRO codes to use for TX */
    uint8_t new_dcro_codes_tx_size;
    /*! VCRO shift value stored in NVM */
    uint16_t nvm_vcro_shift;
    /*! TX frequency table */
    uint16_t freq_table_tx[DCRO_CODE_COUNT_MAX];
    /*! RX frequency table */
    uint16_t freq_table_rx[DCRO_CODE_COUNT_MAX];
    /*! Pulse width offset power tuning */
    int8_t pulse_width_offset;
