...
uint16_t link_tdma_get_preamble_reg_value(uint8_t isi_mitig_pauses, uint32_t preamble_len, sfd_length_t sfd_len_reg_val)
{
    ...
    return SET_PREAMBLE_LENGTH(preamble_len);
}
...
{
    return ((uint64_t)(tdma_sync->free_running_timer - previous_time_stamp) * 1000) / PLL_FREQ_KHZ(CHIP_RATE_20_48_MHZ);
}

uint16_t link_tdma_get_last_timer_increment_us(tdma_sync_t *tdma_sync)
{
    return ((uint64_t)tdma_sync->last_timer_increment * 1000) / PLL_FREQ_KHZ(CHIP_RATE_20_48_MHZ);
}
                                         ...
                                         uint8_t rx_cca_retry_count)
{
    ...
    if (rx_waited_pll_cycles > (tdma_sync->base_target_rx_waited_pll_cycles + cca->retry_time_pll_cycles -
        ...
        for (int8_t i = cca->max_try_count; i >= 0; i--) {
            if ((rx_waited_pll_cycles <
                 (tdma_sync->base_target_rx_waited_pll_cycles + (cca->retry_time_pll_cycles * (i + 1))) -
                     cca->retry_time_pll_cycles / 2) &&
                (rx_waited_pll_cycles >
                 (tdma_sync->base_target_rx_waited_pll_cycles + (cca->retry_time_pll_cycles * i)) -
                     cca->retry_time_pll_cycles / 2)) {
    ...
    if ((tdma_sync->cca_unsync_watchdog_count > CCA_THRESHOLD_WATCHDOG_COUNT) ||
        (tdma_sync->slave_sync_state == STATE_SYNCING)) {
