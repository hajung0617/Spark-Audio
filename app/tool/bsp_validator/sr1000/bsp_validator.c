...
#define WAKEUPE_POSITION  6
#define SLPDEPTH_POSITION 6
#define GO_SLEEP_POSITION 2
...
/* PRIVATE GLOBALS ************************************************************/
static const uint8_t DEFAULT_SFD[] = {0x5E, 0xA6, 0xC1, 0x1D};
static const uint8_t SFD_REGISTER = 0x32;
...
static void validate_transceiver_irq_pin(bsp_radio_t radio_index)
{
    ...
    reg_value = (uint8_t)SET_BIT_OFFSET(GO_SLEEP_POSITION);
    ...
    swc_hal[radio_index].reset_cs();
    swc_hal[radio_index].transfer_full_duplex_blocking(tx_data, rx_data, 6);
    swc_hal[radio_index].set_cs();
    facade_time_delay(1);

    /* Wake up the transceiver. */
    reg_value = (uint8_t)(0 << GO_SLEEP_POSITION);
