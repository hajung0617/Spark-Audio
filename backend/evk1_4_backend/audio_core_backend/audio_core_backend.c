...
sac_processing_t *sac_facade_cdc_processing_init(sac_sample_format_t format, sac_status_t *status)
{
    ...
    cdc_instance.cdc_queue_avg_size = CDC_DEFAULT_QUEUE_AVERAGE;
    cdc_instance.cdc_resampling_length = CDC_DEFAULT_RESAMPLING_LENGTH;
...
{
    return sac_cdc_format_stats(&cdc_instance, buffer, size, status);
}
