...
void (*debug_uart_tx_callback)(void) = NULL;
void (*debug_uart_rx_callback)(void) = NULL;
...
{
    HAL_UART_Transmit(&hlp_uart, data, size, timeout_ms);
}
...
{
    HAL_UART_Receive(&hlp_uart, data, size, timeout_ms);
}
...
{
    if (huart == &hlp_uart) {
        if (debug_uart_rx_callback != NULL) {
...
{
    if (huart == &hlp_uart) {
        if (debug_uart_tx_callback != NULL) {
            debug_uart_tx_callback();
        }
    } else {
