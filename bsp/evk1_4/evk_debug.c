...
void evk_debug_exception_dump(uint32_t *stack_frame)
{
    ...
    evk_uart_debug_write_blocking((uint8_t *)fault_string, out_size, LOG_UART_TIMEOUT_MS);
    out_size = stack_dump_format(stack_frame, stack_string, STACK_DUMP_STRING_SIZE);
    evk_uart_debug_write_blocking((uint8_t *)stack_string, out_size, LOG_UART_TIMEOUT_MS);
#else
    (void)stack_frame;
#endif
}
 ...
 *
 *  @param[in]  stack        Stack pointer pointing to the top of the stack frame to fetch information from.
 *  @param[out] out_string   Pre-allocated char array which will be formatted with stack dump data.
 *  @param[in]  string_size  Size of the pre-allocated out_string. Used to prevent buffer overrun in case of
 *                           insufficient space.
 *
...
static uint16_t stack_dump_format(uint32_t *stack, char *out_string, uint16_t string_size)
{
    ...
    used_string_size += snprintf(out_string, string_size,
                                 "Stack dump:\n\r"
                                 "r0  = 0x%08lx\n\r"
                                 "r1  = 0x%08lx\n\r"
                                 "r2  = 0x%08lx\n\r"
                                 ...
                                 "pc  = 0x%08lx\n\r"
                                 "psr = 0x%08lx\n\r",
                                 stack[r0], stack[r1], stack[r2], stack[r3], stack[r12], stack[lr], stack[pc],
                                 stack[psr]);
 ...
 *
 *  @param[out] out_string   Pre-allocated char array which will be formatted with fault registers data.
 *  @param[in]  string_size  Size of the pre-allocated out_string. Used to prevent buffer overrun in case of
 *                           insufficient space.
 *
...
static uint16_t fault_register_format(char *out_string, uint16_t string_size)
{
    ...
    string_used_size += snprintf(out_string + string_used_size, string_size - string_used_size,
                                 "SCB->HFSR = 0x%08lx\n\r", SCB->HFSR);

    /* Check fault type */
    if (SCB->HFSR & SCB_HFSR_VECTTBL_Msk) {
        string_used_size += snprintf(out_string + string_used_size, string_size - string_used_size,
                                     "A BusFault occurred by reading the vector table for exception processing.\n\r");
    }
    if (SCB->HFSR & SCB_HFSR_FORCED_Msk) {
        /* Forced Hardfault means that it was triggered by a bus fault, a memory fault, or a usage fault */
        string_used_size += snprintf(out_string + string_used_size, string_size - string_used_size,
                                     "Exception was caused by escalation of a configurable fault (FORCED).\n\r");
        string_used_size += snprintf(out_string + string_used_size, string_size - string_used_size,
                                     "SCB->CFSR = 0x%08lx\n\r", SCB->CFSR);
 ...
 *
 *  @param[out] out_string   Pre-allocated char array which will be formatted with memfault data.
 *  @param[in]  string_size  Size of the pre-allocated out_string. Used to prevent buffer overrun in case of
 *                           insufficient space.
 *
...
static uint16_t memfault_format(char *out_string, uint16_t string_size)
{
    ...
    if (cfsr & SCB_CFSR_MMARVALID_Msk) {
        string_used_size += snprintf(out_string + string_used_size, string_size - string_used_size, fault_address,
                                     SCB->MMFAR);
    }
 ...
 *
 *  @param[out] out_string   Pre-allocated char array which will be formatted with busfault data.
 *  @param[in]  string_size  Size of the pre-allocated out_string. Used to prevent buffer overrun in case of
 *                           insufficient space.
 *
 ...
 *
 *  @param[out] out_string   Pre-allocated char array which will be formatted with usagefault data.
 *  @param[in]  string_size  Size of the pre-allocated out_string. Used to prevent buffer overrun in case of
 *                           insufficient space.
 *
...
static uint16_t usagefault_format(char *out_string, uint16_t string_size)
{
    ...
    const char *unaligned = "LDM, STM, LDRD, STRD on unaligned address executed, or single load or store executed when "
                            "enabled to trap (UNALIGNED)\n\r";
    ...
    const char *invstate =
        "Execution in invalid state. E.g. Thumb bit not set in EPSR, or invalid IT state in EPSR (INVSTATE)\n\r";
